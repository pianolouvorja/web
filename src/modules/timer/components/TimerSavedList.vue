<script setup lang="ts">
import { useI18n } from 'vue-i18n'

import { formatElapsedMs } from '../services/timer-format'
import type { TimerTimeFormat } from '../types/timer'

const props = defineProps<{
  items: number[]
  timeFormat: TimerTimeFormat
}>()

const emit = defineEmits<{
  remove: [index: number]
  clear: []
}>()

const { t } = useI18n()

function labelFor(ms: number) {
  return formatElapsedMs(ms, props.timeFormat)
}
</script>

<template>
  <aside
    v-if="items.length > 0"
    class="timer-saved"
  >
    <header class="timer-saved__header">
      <h2 class="timer-saved__title">
        <span class="timer-saved__count">{{ items.length }}</span>
        {{ t('timer.savedTimes') }}
      </h2>
      <button
        type="button"
        class="timer-saved__clear"
        @click="emit('clear')"
      >
        {{ t('timer.clearAll') }}
      </button>
    </header>

    <ul class="timer-saved__list">
      <li
        v-for="(ms, index) in items"
        :key="`${ms}-${index}`"
        class="timer-saved__item"
      >
        <i
          class="ti ti-clock-hour-4"
          aria-hidden="true"
        />
        <span class="timer-saved__value">{{ labelFor(ms) }}</span>
        <button
          type="button"
          class="timer-saved__remove"
          :aria-label="t('timer.removeSaved')"
          @click="emit('remove', index)"
        >
          <i
            class="ti ti-trash"
            aria-hidden="true"
          />
        </button>
      </li>
    </ul>
  </aside>
</template>

<style scoped lang="scss">
.timer-saved {
  display: flex;
  width: 100%;
  max-width: 16rem;
  max-height: 100%;
  flex-direction: column;
  gap: 0.75rem;
  padding: 1rem;
  border-radius: var(--ds-radius-md, 0.75rem);
  background: color-mix(in srgb, var(--ds-color-on-surface) 5%, transparent);
}

.timer-saved__header {
  display: flex;
  flex-direction: column;
  gap: 0.5rem;
}

.timer-saved__title {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  margin: 0;
  color: var(--ds-color-on-surface);
  font-size: 0.95rem;
  font-weight: 700;
}

.timer-saved__count {
  display: inline-flex;
  min-width: 1.4rem;
  height: 1.4rem;
  align-items: center;
  justify-content: center;
  padding: 0 0.35rem;
  border-radius: 9999px;
  background: color-mix(in srgb, var(--ds-color-primary) 22%, transparent);
  color: var(--ds-color-primary);
  font-size: 0.7rem;
}

.timer-saved__clear {
  height: 2rem;
  border: 0;
  border-radius: var(--ds-radius-md, 0.5rem);
  background: color-mix(in srgb, var(--ds-color-error, #ffb4ab) 16%, transparent);
  color: var(--ds-color-error, #ffb4ab);
  cursor: pointer;
  font-size: 0.75rem;
  font-weight: 700;
}

.timer-saved__list {
  display: flex;
  flex: 1;
  flex-direction: column;
  gap: 0.35rem;
  margin: 0;
  padding: 0;
  overflow-y: auto;
  list-style: none;
}

.timer-saved__item {
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.35rem 0.25rem;
  color: var(--ds-color-on-surface);

  > .ti {
    color: var(--ds-color-primary);
    font-size: 1rem;
  }
}

.timer-saved__value {
  flex: 1;
  font-family: ui-monospace, monospace;
  font-size: 0.875rem;
  font-variant-numeric: tabular-nums;
}

.timer-saved__remove {
  display: inline-flex;
  width: 1.75rem;
  height: 1.75rem;
  align-items: center;
  justify-content: center;
  border: 0;
  border-radius: 9999px;
  background: transparent;
  color: var(--ds-color-error, #ffb4ab);
  cursor: pointer;

  &:hover {
    background: color-mix(in srgb, var(--ds-color-error, #ffb4ab) 14%, transparent);
  }
}
</style>
